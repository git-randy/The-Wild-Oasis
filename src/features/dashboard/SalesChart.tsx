...
import { BookingsAfterDateData } from "./blueprints";

const StyledSalesChart = styled(DashboardBox)`
  // Occupy entire container. 1 (beginning) / -1 (end)
  grid-column: 1 / -1;
...
function SalesChart({ bookings, numDays }: SalesChartProps) {
  ...
  const colors = isDarkMode
    ? {
        totalSales: { stroke: "#b1aeec", fill: "#4f46e5" },
        extrasSales: { stroke: "#65eb96", fill: "#22c55e" },
        text: "#e5e7eb",
        background: "#18212f",
      }
    : {
  ...
  return (
    <StyledSalesChart>
      ...
      <ResponsiveContainer height={300} width="100%">
        <AreaChart data={data} margin={{ bottom: 20, left: 20 }}>
